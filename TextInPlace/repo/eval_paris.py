...
args.save_dir = os.path.join("logs", args.save_dir, args.backbone + "_" + args.aggregation, \
                             "paris_75018", start_time.strftime('%Y-%m-%d_%H-%M-%S'))
...
######################################### TEST on TEST SET #########################################
recalls, recalls_str, recalls_rerank, recalls_str_rerank = test.test_text_rerank(
    args, test_ds, model, use_llm=args.use_llm
)
