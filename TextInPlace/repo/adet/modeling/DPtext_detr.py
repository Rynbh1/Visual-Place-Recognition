...
class TransformerPureDetector(nn.Module):
    def __init__(self, cfg):
        ...
        self.denormalizer = lambda x: (x * std + mean)*255
        self.to(self.device)
    ...
    def preprocess_image(self, batched_inputs):
        ...
        """
        images = [self.normalizer(self.denormalizer(x["image"].to(self.device))) for x in batched_inputs]
        tmp_images = [self.normalizer2(self.denormalizer(x["image"].to(self.device))) for x in batched_inputs]
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            output, srcs, hs, encoder_feat, resnet_feat = self.dptext_detr(images)
            ...
            output, srcs, transformer_out, encoder_feat, resnet_feat = self.dptext_detr(images)
            ...
            boxes_results = [Boxes(i.boxes) for i in results]
            batch_len = torch.as_tensor([i.boxes.shape[0] for i in results]) 
            ...
            if rec_img.shape[0]>0:
                ...
                if isinstance(output, tuple):
                    ...
                    cls_logit = None
                scores, pred_ids = output.max(-1)
                pred_ids = pred_ids.split(batch_len.tolist())
                rec_scores = scores.split(batch_len.tolist())
            else:
                pred_ids = torch.empty(len(results), 0)
                rec_scores = torch.empty(len(results), 0)

            processed_results = []
            for results_per_image, input_per_image, image_size, pred_id, rec_score in zip(
                results, batched_inputs, images.image_sizes, pred_ids, rec_scores):
            ...
            return processed_results, resnet_feat
