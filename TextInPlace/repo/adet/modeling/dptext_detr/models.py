...
from adet.layers.deformable_transformer_det import DeformableTransformer_Det
...
class DPText_DETR(nn.Module):
    def __init__(self, cfg, backbone):
        ...
        # self.backbone.adapter = self.adapter
    ...
    def forward(self, samples: NestedTensor):
        ...
        resnet_feat, _ = features[0].decompose()
        ...
        # ctf_feat = self.fpn_adapter(srcs[0], srcs[1], srcs[2], srcs[3])
        # for i in range(len(srcs)):
        #     srcs[i] = srcs[i] + ctf_feat[i]

        # n_pts, embed_dim --> n_q, n_pts, embed_dim
        ctrl_point_embed = self.ctrl_point_embed.weight[None, ...].repeat(self.num_proposals, 1, 1)

        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, encoder_feat = self.transformer(
        ...
        return out, srcs, hs[-1], encoder_feat, resnet_feat
